<template>
  <el-dialog v-model="dialogVisible" title="管理员登录" width="30%" draggable>
    <el-input v-model="text" class="w-50 m-2" type="textarea" placeholder="请输入管理员密码" style="width: 300px" show-word-limit
      autosize />
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogVisible = false">取消</el-button>
        <!-- 传送锚点 -->
        <el-button type="danger" @click="adminLogin">
...
const adminLogin = async () => {
  ...
  axios.post('api/pub/login/admin', JSON.stringify({
    password: text.value
  }), {
    headers: {
...
.total-layout {
  ...
  /* Replace with your image path  register.jpg */
...
.container {
  ...
  /* Slight transparency to see some of the background */
  padding: 40px;
  border-radius: 10px;
  /* Rounded edges */
